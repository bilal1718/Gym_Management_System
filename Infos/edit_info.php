...
$id=$_SESSION['sign']; 
$qb="SELECT * FROM attendence WHERE user_id='$id'";
...
<head>
    ...
    <title>Edit Students List</title>
    <style>
.btn {
...
<body>
    ...
    include_once('../Dashboard/connect.php');
    $id=$_SESSION['sign'];
    $q=mysqli_query($connect,"SELECT signup.id,signup.user_id,signup.user_name,goal_id.goal_name,goal_id.goa_id,goal.goal_id,basic_information.id
    FROM signup,goal_id,goal,basic_information WHERE signup.id=basic_information.id
    AND signup.id=goal.id 
    AND goal.goal_id=goal_id.goa_id AND signup.user_id='$id'");
    ?>
...
<div class="container">
    ...
    while($rowa=mysqli_fetch_object($runa)) {  ?>
      ...
      <?php } ?> 
                                ...
                                <td>
                        <button class="btn" name="update"><i class="fa fa-upload" type='submit' aria-hidden="true"></i></button>
                      </td>
